...
def infection_probabilities(master_df, outbreak_origin_list, G):
    '''
    ...
    '''
    #Initializing a dict to store the infection probabilties for each node that is iterated over 
    infection_probability = {}
    #Creating an empty dataframe to store all the associated information for each infection network 
    infection_prob_table = pd.DataFrame(columns=['Source', 'NetworkSize', 'TreatedCount', 'Reduction', 'NumHCWs', 
    ...
    for source_node, infection_network in outbreak_tree.items():
        ...
        hcw_treated = 0 
        #The source node will always have a probability of 1 so hard coding this is not a problem 
        infection_source_prob = float(1.0)
        ...
        for node in infection_network.nodes():
            ...
            for index, row in filtered_df.iterrows():
                ...
                role = row['Role']
                current_inf_prob = row['InfectionProb']
                ...
                if node == source_node: 
                    ...
                    infection_probability[source_node] = infection_source_prob
                    #Adjusting the cumulative infection odds 
                    infection_sum += infection_source_prob
                    ...
                    if role == 'Patient':
                        ...
                        pt_infection_sum += infection_source_prob
                    continue 
                ...
                #At this point, all healthcare workers should have an infection probability of 1 because none of them are being treated with UDCA.
                #Their infection probabilities will not change until the downstream effects of UDCA are implemented (which is conducted below)
                if role == 'HCW':
                    current_inf_prob = 1
                    hcw_count += 1 
                ...
                if infector == source_node: 
                    ...
                    #Pulling the infection probability of the parent node from the dict using .get()
                    parent_inf_prob = infection_probability.get(infector)
                    try:
                        #Calculating the current node's infection probability relative to the parent 
                        infection_prob = float(parent_inf_prob) * float(current_inf_prob)
                    except TypeError as e:
                        logger.warning(f'failed to compute infection probability for {node}: {e}')
                    ...
                    if treatment_status == False and infection_prob != 1:
                        ...
                        pass
        #Updating the total reduced infection probability once all the nodes branching from this source have been visited
        infection_prob_table.loc[len(infection_prob_table)] = {'Source': source_node, 'NetworkSize': network_length, 
    ...
    return infection_prob_table, outbreak_tree
...
def parallel_processing_files(paired_files, pt_folder, hcw_folder):
    ...
    Return 
        ...
        infection network data = contains information stored from the infection probabilities function 
    '''
    ...
    for file_name1, file_name2 in paired_files:
        ...
        pt_df['InfectionProb'] = float(1)
        pt_df['TreatmentStatus'] = False
        hcw_df['InfectionProb'] = float(1)
        ...
        patient_aggregated = pt_aggregated[['ID', 'Date', 'Infection.Status.CT', 'Community.Infection', 'WardNumber', 'InfectedBy', 'Detected', 'InfectionProb', 'TreatmentStatus', 'Role']]
        #Grouping by ID to make it a unique value, and then adding the information for the other columns according to my needs --> have to convert numerical values to strings (i.e. infection prob)
        ...
        pt_aggregated = pt_aggregated.groupby('ID').agg({'Date':lambda x: x.min(), 'Infection.Status.CT':lambda x: ', '.join(x.unique()), 'Community.Infection':'first', 'WardNumber':'first', 'InfectedBy': 'first', 'Detected': 'first', 'InfectionProb':lambda x: ', '.join(map(str, x.unique())), 'TreatmentStatus': 'first', 'Role': 'first'})
        ...
        hcw_aggregated = hcw_aggregated[['ID', 'Date', 'Infection.Status.CT', 'Community.Infection', 'InfectedBy', 'Detected', 'InfectionProb', 'TreatmentStatus', 'Role']]
        hcw_aggregated = hcw_aggregated.groupby('ID').agg({'Date':lambda x: x.min(), 'Infection.Status.CT':lambda x: ', '.join(x.unique()), 'Community.Infection':'first', 'InfectedBy': 'first', 'Detected': 'first', 'InfectionProb':lambda x: ', '.join(map(str,x.unique())), 'TreatmentStatus': 'first', 'Role': 'first'})
        ...
        #Ensuring the index's are correct because its creating problems when updating the infection probabilities based on index      
        ...
        infection_networks, dfs_networks = infection_probabilities(merged_aggregated, outbreak_starter, G)
        ...
        for root, network in dfs_networks.items():
            ...
            for edge in network.edges():
                ...
                for _, row in filtered_df_infector.iterrows():
                    #Pulls the node associated with the infector from the network plot and adds their infection probability 
                    dfs_plot.nodes[infector]['InfectionProb'] = row['InfectionProb']
                ...
                for _, row in filtered_df_infected.iterrows():
                    #Infection probability is added
                    dfs_plot.nodes[infected]['InfectionProb'] = row['InfectionProb']
