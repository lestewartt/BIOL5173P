...
def summary_subsets(network_file_paths):
    ...
    for path in network_file_paths: 
        ...
        #The total infection prob pre treatment is also synonymous to the total number of individuals from the simulation included 
        total_prob_pre_treatment = infection_df['NetworkSize'].sum()
        total_prob_post_treatment = infection_df['Reduction'].sum()
        difference_in_reduction = total_prob_pre_treatment - total_prob_post_treatment
        #Percent reduction of infection odds across the entire hospital 
        percent_reduction = (difference_in_reduction/total_prob_pre_treatment) * 100
        ...
        infection_network_analysis.loc[len(infection_network_analysis)] = {'SimulationNumber': sim_number, 'PercentReduction': percent_reduction, 
                                                                           'PropCasesReduced': prop_reduced, 'InitialInf': total_prob_pre_treatment, 
                                                                           'AdjustedInf': total_prob_post_treatment, 'NumHCWs': num_hcws, 
...
#print(parameter_subset_1.describe())
